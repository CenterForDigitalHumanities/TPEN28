...
public class overloadLoader
{
    ...
    public static void setBlobs() throws SQLException
    {
        ...
        {
            try {
                Vector <blob> b=getBlobs("/usr/data/"+imgs.getString(1).replace(".jpg", "")+".txt");
                ...
                getLogger(overloadLoader.class.getName()).log(SEVERE, null, ex);
    ...
    public static void setCharCounts() throws SQLException
    {
        ...
        out.print("getting distinct images\n");
        out.flush();
        ResultSet imgs=select.executeQuery();
        while (imgs.next())
        {
            out.print("getting blobs for image "+imgs.getString(1)+"\n");
            out.flush();
    ...
    public overloadLoader(File dir) throws FileNotFoundException, IOException, SQLException
    {
        ...
        for (File file : files) {
            ...
            while (r.ready()) {
                ...
                int count1 = parseInt(imageNames[0].split(":")[1]);
                String img2=imageNames[1].split(":")[0];
                int count2 = parseInt(imageNames[1].split(":")[1]);
    ...
    public overloadLoader(File dir, Hashtable <Integer,Integer> h,Hashtable <Integer,Integer> h2, Boolean drilldown) throws FileNotFoundException, IOException, SQLException
    {
      ...
      BufferedReader r;
        ...
        PreparedStatement blobInsertStatement=j.prepareStatement(blobsInsert);
            try (FileWriter dataFile = new FileWriter(new File("/usr/ddata.dat"))) {
                for (File file : files) {
                    h=null;
                    r = new BufferedReader(new FileReader(file));
                    out.print(file.getName() + "\n");
                    ...
                    LinkedList<Future> set = new LinkedList<>();
                    ...
                    executor = newFixedThreadPool(taskCount, new prioritythread());
                    while (r.ready()) {
                        buff=r.readLine();
                        ...
                        //blob id number, counts from 1-the number of blobs on the page
                        try {
                            int count1 = parseInt(colon.split(imageNames[0])[1].split(";")[0]);
                            String img2=colon.split(imageNames[1])[0];
                            int count2 = parseInt(colon.split(imageNames[1])[1].split(";")[0]);
                            if(img1.compareTo(img2)!=0)
                            {
                                /*if(h==null)
                                {
