...
import static detectimages.blob.readFileIntoArray;
        ...
        try (Connection j = getConnection()) {
            ...
            while (rs.next()) {
                if (img2.compareTo(rs.getString(1)) != 0) {
                    img2 = rs.getString(1);
                    url += ("<br>" + img2 + ":");
                }      int folioNum = parseInt(rs.getString(1).replace(".jpg", "").split("/")[rs.getString(1).replace(".jpg", "").split("/").length - 1]);
   ...
   public matchLocater(int img1, int blob1, String express) throws FileNotFoundException, IOException, SQLException {
      ...
      File f = new File(getRbTok("PALEOTEMPDIR") + getRbTok("PALEODATADIR") + "/" + ms.getID() + "/" + img1 + ".txt .txt");
        try (BufferedReader b = new BufferedReader(new FileReader(f))) {
            ...
            String oldpage = "";
            Pattern colon = compile(":");
            Pattern semicolon = compile(";");
            Pattern slash = compile("/");
            Pattern quote = compile("\"");
            for (String buff : lines) {
                buff = buff.replace("\"", "");
                ...
                int blob = parseInt(parts1);
                if (blob1 == 0 || blob1 == blob) {
                    if (oldpage.compareTo(parts2) != 0) {
                        Folio tmp = new Folio(parseInt(parts2));
      ...
      File f = new File(getRbTok("PALEOTEMPDIR") + getRbTok("PALEODATADIR") + "/" + ms.getID() + "/" + img1 + ".txt .txt");
        try (BufferedReader b = new BufferedReader(new FileReader(f))) {
            ...
            for (String buff : lines) {
                ...
                int matchLevel = parseInt(parts4);
                int blob = parseInt(parts1);
                if (blob1 == 0 || blob1 == blob) {
                    if (oldpage.compareTo(parts2) != 0) {
                        Folio tmp = new Folio(parseInt(parts2));
        ...
        try (BufferedReader b = new BufferedReader(new FileReader(f))) {
            ...
            while (b.ready()) {
                ...
                if (blob1 == 0 || blob1 == blob) {
                    ...
                    //this is what they wanted
                    int folioNum = parseInt(parts2);
                    out.print("folio:" + folioNum + "\n");
                    ...
                    if (blob1 == 0 || blob1 == blob) {
                        if (oldpage.compareTo(parts2) != 0) {
                            if (oldpage.compareTo("") != 0) {
                                url += "<div class='result' matchLevel='" + maxLevel + "'>" + new Folio(parseInt(oldpage)).getPageName() + ":" + tmpResult + "</div>";
                                tmpResult = "";
                            }
                            Folio tmp = new Folio(parseInt(parts2));
                            oldpage = parts2;
                        ...
                        if (matchLevel > maxLevel) {
                            ...
                            String folioNumber = parts2;
                            blobGetter thisBlob = new blobGetter(folioNumber, parseInt(parts3));
...
tmpResult += "<span href=\"paleo.jsp?highlightblob=true&p=" + folioNumber + "&orig=1&blob=" + parts3 + "\" matchLevel='" + matchLevel + "'><img src=\"characterImage?page=" + folio.getFolioNumber() + "&blob=" + parts3 + "\"/></span>\n";
   ...
   public static String getBlobMatchCousins(String img1, int blob1) throws SQLException {
      ...
      String oldimg = "";
        for (String blob : blobs) {
            try {
                String tmp = blob;
                int num = parseInt(tmp.split(".jpg")[1]);
                tmp = tmp.split(".jpg")[0];
   ...
   public static void main(String[] args) {
      try {
         ...
         for (int i = 0; i < 100; i++) {
                out.print("" + i + ": " + m.getBlobMatchCount(i) + "\n");
         }
            out.print(m.url);
      } catch (SQLException | IOException ex) {
            getLogger(matchLocater.class.getName()).log(SEVERE, null, ex);
      }
   }
}
