...
/**
 * @author Jon Deering
 * Used for tasks requiring the management of all of the folios in a collection at once
 * (for building listings of them, showing thumbnails, counting them, and the like) This class was created
 * before the manuscript class was created, but is still used in some cases for managing multiple folios.
 */
public class FolioSet {
    ...
    * @throws SQLException
    */
   public FolioSet(Connection conn, String archive, String collection) throws SQLException {
      folios = new Vector();
      Connection j = null;
      try (PreparedStatement stmt = conn.prepareStatement("SELECT * FROM folios WHERE archive = ? AND collection = ?")) {
   ...
   /**
    * Zero page the last number in a string to 4 places, for use in sorting images when the metadata is
    * messy (non-uniform number of digits)
    *
