...
/**
 *
 * Represents a user of the application. This handles creating new accounts,
 * authenticating users, and easily getting data attached to the account.
 */
public class User {
    ...
    /**
     * Returns the last Folio this person transcribed which was NOT in a Project, -1
    ...
    /**
     * Returns a date object representing the last time this user was active, or the
     * epoc (time 0) if they have never been active
     */
    ...
    /**
     * return the number of minutes since last activity, or -1 if they have never
     * been active
     */
    ...
    /**
     * Returns the last Folio this person transcribed, -1 indicates nothing was
     * found
     */
    ...
    /**
     * Returns the last Project the user modified a page in, -1 indicates nothing
     * was found
     */
    ...
    /**
     * Get the username (part of email address before @) of the owner of this
     * comment
     * 
     * @return
     */
     ...
     * @param smt    A dummy param added because a single String constructor already
     *               exists.
     * @throws SQLException
     */
    ...
    /**
     * Populate the user's info from the database if and only if the password
     * provided is correct.
     * 
     * @param id   user's unique id
    ...
    /**
     * Sign up a new user. Returns 0 for success, 1 for failure to create user
     * account, and 2 for success creating user but failure to send notification
     * email, likely due to being run on a test system with no email server
     * 
    ...
    /**
     * Populate a user object based on their username(email). The idea is that if a
     * user needs to be invited to a group it may be done via their email
     * 
    ...
    /**
     * Create a new user account that authenticates via uname and password. Password
    ...
    /**
     * Deactivate the user by setting this user's password to empty, which prevents
     * them logging in until they are approved by an administrator
     * 
     * @throws SQLException
     */
    ...
    /**
     * Return an array of users who have blank passwords and openids, which
     * indicates they are awaiting approval by an admin
     * 
    ...
    /**
     * Save this user record only if it is a new user. Does nothing if the user
     * already exists.
     * 
    ...
    public void commit(String pass) throws SQLException {
        ...
        try {
            ...
            if (this.exists()) {
                /*
                 * qry=j.
                 * prepareStatement("update users set Uname=?, lname=?, fname=? where UID=?");
                 * qry.setInt(4, UID); qry.setString(1, Uname); qry.setString(2, lname);
                 * qry.setString(3, fname); qry.execute();
                 */
            } else {
    ...
    /**
     * Create a new sudo random password for this user, set their password to match
     * it, and return the text of the new password
     * 
    ...
    /**
     * This sets the last time the user was active to the current time. Used for
     * determining who is online, and keeping track of active vs inactive users
     */
    ...
    public Project[] getUserProjects() throws SQLException {
        ...
        try {
            ...
            ps.setInt(2, UID);
            ResultSet rs = ps.executeQuery();
            Stack<Integer> projectIDs = new Stack<Integer>();
            while (rs.next()) {
     ...
     * 
     * @return true if they havent been approved yet. This sets their password to ""
     *         which is an impossible sha-1 hash
     * @throws SQLException
     */
    ...
    /**
     * Return an array of all manuscripts this person controls. If they are an
     * admin, that is every restricted ms
     */
    public Manuscript[] getUserControlledManuscripts() throws SQLException {
        Manuscript[] toret = new Manuscript[0];
        // all restricted manuscripts have the user id of the controller in the
        // restricted field
