...
//        $("#workspace,#imgBottom").fadeOut();
      ...
      var correctHeight = (topImg.height() > Page.height()) ? -999 : firstLine.attr("lineheight") * topImg.height() / 1000;
      if ((Math.abs(firstLine.height() - correctHeight) > 2.5) || (topImg.height() > Page.height())) {
...
//        this.organizePage(e);
    ...
    if (afterThisID === -1) {
      ...
      afterThisID = $(".transcriptlet").eq(-1).attr("data-lineid") || -1;
      $(".transcriptlet").each(function(index) {
        if ($(this).find('lineLeft') > newX) {
          afterThisID = (index > 0) ? $(this).prev('.transcriptlet').attr("data-lineid") : -1;
          return false;
        }
      });
            isNotColumn = false;
...
//    }, '.annotation');
    ...
    $("#parseOptions").children("span").click(function(event){
      ...
      // restore events on .lines
      $("#imgTop").children(".line").css({
        'pointer-events': '',
        'z-index': ''
      });
      return true;
...
//FIXME this does not actualy detect success
    ...
    var reload = window.setInterval(function() {
      if ($('#bookmark').siblings('div').length + $.active === 0) {
